...
#train.info()
survived_index = train.columns.get_loc("Survived")
...
#X = np.array(train.drop(train.columns[survived_index], axis=1).astype(float))
#print(X)
#y = np.array(train['Survived'])
...
kmeans = KMeans(algorithm='lloyd', copy_x=True, init='k-means++', max_iter=600,
 n_clusters=2, n_init=100, random_state=10, tol=0.0001, verbose=0)
scaler = MinMaxScaler()
